...
class SecurePRNG:
    def __init__(self, seed_int):
        self.state = hashlib.sha256(seed_int.to_bytes((seed_int.bit_length()+7)//8, 'big')).digest()
...
class Entity:
    def __init__(self, name):
        self.name = name
        self.private_key = secrets.randbelow(P-2) + 1  # random private key 1 <= x <= P-1
...
class Mallory:
    ...
    def intercept(self, sender, recipient, payload):
        if isinstance(payload, str) and payload.startswith("0x"):
            ...
            return self.public_hex  # Send Mallory's key instead

        if isinstance(payload, bytes):
            print(f"[MALLORY] Intercepting Encrypted Message from {sender}...")
            if sender == "Alice":
                plaintext = xor_crypt(payload, self.alice_prng)
                print(f"[MALLORY] Original message: {plaintext.decode()}")
                modified = plaintext.replace(b"9pm", b"3am")  # Example modification
